...
# grid search model settings
# find best model settings for that set of features
# use model to predict on train.test split
# keep maximal model settings and result
quantile.settings <- c(0.25, 0.5, 0.75, 0.9)
for(j in 1:length(quantile.settings)){
  for( i in 2:ncol(att.importance)){
    print(i)
    
    # identify valid features and create formula
    q <- quantile(att.importance[,i], quantile.settings[j])
    #   q <-0
...
# once maximal model settings are identified, train the 
# model using the maximal feature and model settings
...
for( i in 2:ncol(att.importance)){
  ...
  # identify valid features and create formula
  q <- quantile(att.importance[,i], 0.75)
#   q <-0
  valid.features <- as.character(att.importance$X[att.importance[,i] > q])
  f <- formula(paste(
    names(train.y)[i], " ~ ",
